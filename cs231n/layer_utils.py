...
def conv_sbn_relu_forward(x, w, b, gamma, beta, conv_param, sbn_param):
  a, conv_cache = conv_forward_fast(x,w,b,conv_param);
...
def conv_sbn_relu_backward(dout, cache):
  ...
  da = relu_backward(dout, relu_cache);
  db, dgamma, dbeta = spatial_batchnorm_backward(da, sbn_cache);
  dx, dw, db = conv_backward_fast(db, conv_cache);
  return dx, dw, db, dgamma, dbeta;

def conv_sbn_relu_pool_forward(x, w, b, gamma, beta, conv_param, sbn_param, pool_param):
  a, cache_csr = conv_sbn_relu_forward(x, w, b, gamma, beta, conv_param, sbn_param);
  out, cache_pool = max_pool_forward_fast(a, pool_param);
  cache = (cache_csr, cache_pool);
  return out, cache;

def conv_sbn_relu_pool_backward(dout, cache):
  cache_csr, cache_pool = cache;
  dv = max_pool_backward_fast(dout, cache_pool);
  dx, dw, db, dgamma, dbeta = conv_sbn_relu_backward(dv, cache_csr);
  return dx, dw, db, dgamma, dbeta;
